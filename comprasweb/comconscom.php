...
<body>
<h1>CONSULTA DEL COMPRADOR - alexander_suntaxi_acosta</h1>
<?php
...
function obtenecliente($db) {
	...
	$sql = "SELECT DISTINCT COMPRA.NIF  FROM CLIENTE,COMPRA where CLIENTE.nif=COMPRA.nif";
	$sql =strtoupper($sql);
...
function obteneInformacion($db,$CLIENTE){
    ...
    $sql = "SELECT DISTINCT PRODUCTO.ID_PRODUCTO,PRODUCTO.NOMBRE,PRODUCTO.PRECIO FROM COMPRA,PRODUCTO,CLIENTE where COMPRA.NIF='$CLIENTE' AND COMPRA.NIF=CLIENTE.NIF AND PRODUCTO.ID_PRODUCTO=COMPRA.ID_PRODUCTO AND 
	COMPRA.FECHA_COMPRA BETWEEN '$dateI' AND '$dateF'" ;
    $resultado = mysqli_query($db, $sql);
    if ($resultado) {
        while ($row = mysqli_fetch_assoc($resultado)) {
           echo "<tr>";
